...
import com.commandcenter.common.utils.Page;
...
public class PageInterceptor implements Interceptor {
    ...
    public void setDatabaseType(String databaseType) {
        if (!databaseType.equalsIgnoreCase(MYSQL) && !databaseType.equalsIgnoreCase(ORACLE)&& !databaseType.equalsIgnoreCase(POSTGRES)) {
...
//                log.info("不是分页,查询sql: " + retSqlReplaceSpace(sql));
            ...
            if (log.isInfoEnabled()) {
                log.info("分页时, 生成分页pageSql: " + retSqlReplaceSpace(pageSql));
            }
    ...
    protected String buildPageSql(Page<?> page, String sql) {
        if (MYSQL.equalsIgnoreCase(databaseType)) {
            ...
            return buildPostgresPageSql(page, sql);
        }
        return sql;
    }
     ...
     * @return
     */
    protected String buildPostgresPageSql(Page<?> page, String sql) {
        // 计算第一条记录的位置，postgres中记录的位置是从0开始的。
        // postgres 只需要计算开始条数即可
        StringBuilder sb = new StringBuilder(sql);
        sb.insert(0, "select u.* from (").append(") u where 1=1  ");
        int offset = (page.getPageNo() - 1) * page.getPageSize();
        ...
        sb.append(" limit ").append(page.getPageSize()).append(" offset ").append(offset);
        return sb.toString();
    }
     ...
     * @throws SQLException
     */
    protected void queryTotalRecord(Page<?> page, Object parameterObject, MappedStatement mappedStatement, Connection connection) throws Exception {
        BoundSql boundSql = mappedStatement.getBoundSql(parameterObject);
        String sql = boundSql.getSql();
        String countSql = this.buildCountSql(sql);
        if (log.isInfoEnabled()) {
            log.info("分页时, 生成countSql: " + retSqlReplaceSpace(countSql));
        }

        List<ParameterMapping> parameterMappings = boundSql.getParameterMappings();
        BoundSql countBoundSql = new BoundSql(mappedStatement.getConfiguration(), countSql, parameterMappings, parameterObject);
        /**
         * g20416 20170522 重写mybatis分页 解决foreach标签参数传递为空的问题
         * */
        ...
        /**
         * r25437 20180926 修改springboot绑定参数时，foreach参数为null问题
         */
        Field additionalField = ReflectUtil.getField(boundSql, "additionalParameters");
        if (additionalField != null) {
            Map<String, Object> map = (Map<String, Object>) ReflectUtil.getFieldValue(boundSql, "additionalParameters");
            ReflectUtil.setValueByFieldName(countBoundSql, "additionalParameters", map);
        }
    ...
    /**
     * 此方法描述的是：格式化String 字符串方法 去掉多余空格
     *
     * @author: g20416
     * @version: 2016年10月26日 上午11:55:44
     */

    public static String retSqlReplaceSpace(String sql) {
        String keywords = sql;
        StringBuilder result = new StringBuilder();
        // 前一个是否为空格，默认第一个不是
        for (int i = 0; i < keywords.length(); i++) {
            String nowCharToStr = "";
            String beforeCharToStr = "";
            /**
             * 逻辑如下: 1.i=0时 如果当前为空格 则拼接"" 2.i>0时 如果当前为空格 且 前一个不是空格 则拼接一个空格 如果当前不是空格 拼接原有值 否则拼接""
             */
            if (i == 0) {

        /* 当前字符是否为空格 */
                nowCharToStr = String.valueOf(keywords.charAt(i)).trim();

                if ("".equals(nowCharToStr)) {
                    result.append("");
                } else {
                    result.append(nowCharToStr);
                }
            } else {
        /* 当前字符是否为空格 */
                nowCharToStr = String.valueOf(keywords.charAt(i)).trim();
        /* 前一个字符是否为空格 */
                beforeCharToStr = String.valueOf(keywords.charAt(i - 1)).trim();

                if ("".equals(nowCharToStr) && !"".equals(beforeCharToStr)) {
                    result.append(" ");
                } else if (!"".equals(nowCharToStr)) {
                    result.append(nowCharToStr);
                } else {
                    result.append("");
                }
